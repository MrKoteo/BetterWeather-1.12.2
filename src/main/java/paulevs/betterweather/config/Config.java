...
public class Config {
	...
	public void addEntry(String name, boolean value, String... comments) {
		...
		entries.put(name, new ConfigEntry<>(name, value, Arrays.asList(comments)));
		order.add(name);
	}
	...
	/**
	 * Replace a stored value at runtime. Does not persist on its own - call {@link #flush()}.
	 * @param name entry name
	 * @param value new value (must match the entry's stored type)
	 */
	public void set(String name, Object value) {
		ConfigEntry<?> entry = entries.get(name);
		if (entry != null) entry.setValueUnchecked(value);
	}

	/** Write the current values back to disk (used after live config edits). */
	public void flush() {
		writeFile();
	}
